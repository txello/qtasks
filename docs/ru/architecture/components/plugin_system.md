...
* ключ (str) — имя триггера;
* значение (list[BasePluginRegistry]) — список зарегистрированных плагинов для
данного триггера.
* Список плагинов внутри каждого триггера отсортирован по приоритету.
* Сортировка выполняется по возрастанию priority (от меньшего значения к большему).
* Это гарантирует предсказуемый порядок выполнения плагинов.
...
Плагины внутри компонентов не хранятся напрямую.
Каждый плагин оборачивается в объект реестра плагина.

Базовым типом реестра является:

```python
BasePluginRegistry
```

Это абстрактный класс, от которого реализованы:

* AsyncPluginRegistry
* SyncPluginRegistry

Эти классы управляют вызовом соответствующих типов плагинов.

## Поля BasePluginRegistry

Каждый реестр плагина содержит следующие поля:

| Поле       | Тип    | Описание                         |
| ---------- | ------ | -------------------------------- |
| `name`     | `str`  | Имя плагина (`plugin.name`)      |
| `plugin`   | `Any`  | Экземпляр плагина                |
| `enabled`  | `bool` | Разрешён ли запуск плагина       |
| `priority` | `int`  | Приоритет выполнения             |
| `cache`    | `dict` | Кэш данных для `_plugin_trigger` |
...
Плагины подключаются через метод:
...
1. создаётся экземпляр `PluginRegistry`;
2. он добавляется в `self.plugins[trigger_name]`;
3. список плагинов для триггера сортируется по `priority`.
...
Плагинная система QTasks представляет собой
**компонентно-ориентированный механизм расширения**, в котором:

* плагины регистрируются через `add_plugin()`;
* управляются через `BasePluginRegistry`;
* хранятся в `dict[str, list[BasePluginRegistry]]`;
* выполняются через `_plugin_trigger()`.

Такой подход позволяет безопасно расширять систему и внедрять новую
функциональность без изменения архитектуры компонентов.
